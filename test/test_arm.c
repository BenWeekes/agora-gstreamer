...
int main(int argc, char *argv[]) {
  ...
  snprintf (video_pipe_str, MAX_BUFFER/4, " v4l2src device=/dev/video0 ! video/x-raw,width=640,height=480,framerate=30/1 ! videoconvert ! x264enc key-int-max=30 tune=zerolatency !  video/x-h264,format=I420, profile=baseline !  h264parse  ! queue ! agoraioudp appid=%s channel=%s outport=7372 inport=7373 verbose=false ! queue ! omxh264dec ! nvvidconv ! video/x-raw, format=I420, width=1280,height=720 ! queue ! nvdrmvideosink sync=false -e", appid, channel);
