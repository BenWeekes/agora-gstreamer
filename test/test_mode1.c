...
int main(int argc, char *argv[]) {
  ...
  if(mode==1){
      snprintf (audio_out_pipe_str, MAX_BUFFER/4,"alsasrc ! audio/x-raw,format=S16LE,channels=1,rate=48000 ! audioconvert ! audioresample quality=8 ! udpsink host=127.0.0.1 port=7373");
      //snprintf (audio_out_pipe_str, MAX_BUFFER/4,"audiotestsrc wave=sine ! udpsink host=127.0.0.1 port=7373");
  }
  else{
  ...
  pipeline = gst_parse_launch (complete_pipe_str,NULL);

  /* Start playing */
  gst_element_set_state (pipeline, GST_STATE_PLAYING);

  //We have to set udpsink0 in play mode so that the whole pipe works in case of mode 1
  set_element_state(pipeline, "udpsink0", GST_STATE_PLAYING);
  print_element_states(pipeline);
