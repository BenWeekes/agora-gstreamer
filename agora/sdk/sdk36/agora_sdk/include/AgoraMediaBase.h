...
struct AudioPcmFrame {
  ...
  int64_t handle_timing[kMaxHandleTimingCount];
  ...
  AudioPcmFrame& operator=(const AudioPcmFrame& src) {
    ...
    this->source_type = src.source_type;
    ...
    memcpy(this->handle_timing, src.handle_timing, sizeof(handle_timing));
  ...
  AudioPcmFrame()
    ...
    memset(handle_timing, 0, sizeof(handle_timing));
  }
...
class IAudioFrameObserverBase {
 public:
  ...
  struct AudioFrame {
    ...
    /**
     * The frame source type indicate the frame's origin.
     */
    int source_type;
    /**
     * The timestamps(us) when this frame processing in specfic timgin on audio send pipeline.
     */
    int64_t handle_time_us[MAX_HANDLE_TIME_CNT];
